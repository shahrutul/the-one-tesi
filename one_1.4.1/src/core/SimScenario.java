...
public class SimScenario implements Serializable {
	...
	/** 
	 * Adds a new query event listener for all nodes.
	 * @param ql The listener
	 */
	public void addQueryListener(FileEventListener ql) {
		this.queryListeners.add(ql);
	}
	
	/**
	 * Returns the list of registered query event listeners
	 * @return the list of registered query event listeners
	 */
	public List<FileEventListener> getQueryListeners() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				DTNHost host = new DTNHost(this.messageListeners, 
						this.movementListeners, this.queryListeners, gid, mmNetInterfaces, comBus, 

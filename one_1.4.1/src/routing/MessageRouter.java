...
public abstract class MessageRouter {
	...
	private List<FileEventListener> qListeners;
	 ...
	 */
	public void init(DTNHost host, List<MessageListener> mListeners, List<FileEventListener> qListeners) {
		this.incomingMessages = new HashMap<String, Message>();
		this.messages = new HashMap<String, Message>();
		this.deliveredMessages = new HashMap<String, Message>();
		this.mListeners = mListeners;
		this.qListeners = qListeners;
		this.host = host;
	}
	
	public void init(DTNHost host, List<MessageListener> mListeners){
		init(host, mListeners, new ArrayList<FileEventListener>());
	}
	...
	public RoutingInfo getRoutingInfo() {
		...
		if(SimScenario.getInstance().simulateFiles() && host.hasFileCapability()){
			...
			for(DTNFile file : host.getFileSystem().getFileCollection()){
				files.addMoreInfo(new RoutingInfo(file.getFilename() + " (100)%"));
			}
	...
	public void notifyfileRequestSatisfied(DTNHost where, String filehash, boolean selfSatisfied){
		for (FileEventListener ql : this.qListeners) {
			ql.fileRequestSatisfied(where, filehash, selfSatisfied);
		}
	}
	
	public void notifyfileRequestDelegated(DTNHost from, DTNHost to, String filehash){
		for (FileEventListener ql : this.qListeners) {
			ql.fileRequestDelegated(from, to, filehash);
		}
	}

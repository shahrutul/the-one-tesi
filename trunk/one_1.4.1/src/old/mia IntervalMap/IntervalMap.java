...
public class IntervalMap{
     ...
     * CopyConstructor.
     *
     * @param otherMap the map to copy
     */
    public IntervalMap(IntervalMap otherMap) {
        this.length = otherMap.length;
        intervals = new Vector<Interval>();
        for(Interval i: otherMap.intervals){
        	intervals.addElement(new Interval(i.getLowerEnd(),i.getUpperEnd(), i.isFreeSpace()));
        }        
    }
	...
	private void cut(int intervalIndex, int cutPoint){
		Interval i = intervals.remove(intervalIndex);
		Interval[] news = i.split(cutPoint);
		orderedInsert(news[0]);
		orderedInsert(news[1]);
	}
    ...
    public int cumputeRawPointer() {
        ...
        int d = (int)Math.floor(
                    ((1+2*firstFreeInterval())*
                    length-1)/
                    (2*intervals.size())+0.5f);
        cut(getContainingIndex(d), d);
        return d;

    }

	private int firstFreeInterval() {
		for(int i=0; i<intervals.size(); i++){
			if(intervals.get(i).isFreeSpace()){
				return i;
			}
	...
	public int[] getStartingPoints(int n) {	
		...
		for(int p=1; p<n; p++){
			int d = (1+2*p)*(length-1)/(2*(intervals.size()));
			points[p] = d;
		}
		return points;
	}

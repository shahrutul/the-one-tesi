...
M2MShare operates on devices with limited memory and calculation resources. Each device have to handle simultaneously incoming requests for upload and delegations from other peers and user request issued on the device itself. If these requests would be executed in parallel, then each one would require a separate execution flow. With the limited device resources, this can cause a large memory usage and consequently a probable application crash. 
\\
...
When a task is created or received from another peer delegation, it is queued in the related queue and scheduled for later execution. Tasks are executed sequentially, and the execution order depends on task type. Some queues have priority over other queues, i.e. 

viene data priorità ai tasks creati dall'utente
